...
def add_user():
    ...
    try:
        ...
        email_list, user_list = User.get_emails_and_users(full_path + '/json_files/UserData.json')
        ...
        if isinstance(resp, User):
            ...
            User.user_to_json(user_list, full_path + '/json_files/UserData.json')
...
@app.route('/edit_service', methods=['PUT'])  # TIRAR CONFIRMA��O DE SENHA! DESNECESSARIO
def edit_service():
...
def admin_options():
    ...
    try:
        ...
        if Admin.admin_checker(admin_login, admin_pw1, admin_pw2,
                               full_path + '/json_files/AdminEspecifications.json'):
            if validator == 'exists':
                if admin_opt == 'ACT':
